...
function show_myself(){
   $.ajax({
       url: "http://192.168.0.77:8080/user/getUser",
       type: "post",
       data:{"uId" : uId},
       dataType: "json",
         ...
         $('.address_bottom_box').html('')
           for(i =0 ;i < data.length;i++){
               str +='<div class="address-news">'+
                           ...
                           '</div>'+
                           '<div  class="operate">'+
                                 '<div  data-toggle="modal" data-target="#corretDress" class="corretBtn">修改</div>'+
                                 '<div>删除</div>'+
                           '</div>'+
                        '</div>'
...
$(".dress").blur(function(){
   $.ajax({
      url: "http://192.168.0.77:8080/address/addressCoordinates",
      type: "post",
      data:{"address":$(".dress").val()},
      // dataType: "json",
      success: function(data){
         var p1 = [121.655404,29.884467];
         var p2 = data
         // 返回 p1 到 p2 间的地面距离，单位：米
         var dis = AMap.GeometryUtil.distance(p1, p2);
         if(dis >3000){
            layer.msg('非常抱歉，您的位置超出配送范围');
   ...
   if($('#person_newPhone3').val() == ''){
      ...
      layer.alert('请填写验证码');
         ...
         $.ajax({
            ...
            type: "post",
            data:{type:'email',oldNumber:$('#person_phone').text(),number:$('#person_newPhone3').val(),uId:$('#uId').val(),code:$('#code3').val()},
            dataType: "json",
            success: function(data){
               if(data == false){
                  layer.msg('修改失败');
...
$("#submitByEmail").click(function(){
   if($('#person_newPhone4').val() == ''){
      ...
      $.ajax({
         url: "http://192.168.0.77:8080/user/verify",
         type: "post",
         data:{type:'mail',number:$('#person_newPhone4').val(),oldMail:$('#person_email').text(),uId:$('#uId').val(),code:$('#code4').val()},
         dataType: "json",
         success: function(data){
            if(data == 200){
               layer.msg('修改成功');
   ...
   if($('#person_newMail').val() == ''){
      ...
      layer.alert('请填写验证码');
         ...
         $.ajax({
            url: "http://192.168.0.77:8080/user/verify",
            type: "post",
            data:{type:'number',mail:$('#person_newMail').val(),oldNumber:$('#person_phone').text(),uId:$('#uId').val(),code:$('#code3').val()},
            dataType: "json",
            success: function(data){
               if(data == false){
                  layer.msg('修改失败');
...
$("#submitByEmail1").click(function(){
   if($('#person_newMail1').val() == ''){
      ...
      $.ajax({
         url: "http://192.168.0.77:8080/user/verify",
         type: "post",
         data:{type:'mail',mail:$('#person_newMail1').val(),oldMail:$('#person_email').text(),uId:$('#uId').val(),code:$('#code1').val()},
         dataType: "json",
         success: function(data){
            layer.msg('修改成功');
...
$(".sure").click(function(){
   if($('#oldPassword').val() == ''){
      ...
      $.ajax({
         ...
         type: "post",
         data:{uId:$('#uId').val(),oldPasd:$('#oldPassword').val(),newPasd:$('#newPassword').val()},
         dataType: "json",
         success: function(data){
            layer.msg('修改成功');
         },
