...
$(function () {
    ...
    var nc_token = ["FFFF0N000000000093E3", (new Date()).getTime(), Math.random()].join(":");
    var NC_Opt =
        {
            renderTo: "#your-dom-id",
            appkey: "FFFF0N000000000093E3",
            scene: "nc_activity",
            token: nc_token,
            customWidth: 300,
            trans: {"key1": "code0"},
    ...
    $(document).ready(function () {
        ...
        let yzm = () => {
            if ($("#exampleInputPhone").val()) {
                ...
                $.ajax({
                    type: "POST",   //提交的方法
                    url: "http://192.168.0.118:8080/sendCode", //验证码接口
        ...
        $("#check").click(function () {
            ...
            if (tabId === 1) {
                if ($("#exampleInputEmail").val() && $("#exampleInputPassword").val()) {
                    ...
                    $.ajax({
                        type: "POST",   //提交的方法
                        url: "http://192.168.0.118:8080/user/namelogin", //提交的地址
                        data: obj,
                ...
                if ($("#exampleInputPhone").val() && $("#exampleInputCheck").val()) {
                    ...
                    $.ajax({
                        type: "POST",   //提交的方法
                        url: "http://192.168.0.118:8080/user/phoneLogin", //提交的地址
                        data: obj,
                        success: function (userId) {  //成功
                            if (userId === "") {
                                Swal.fire({
